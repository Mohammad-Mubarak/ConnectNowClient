...
export default function VideoCard({
  ...
  return (
    <div className={`relative rounded-2xl overflow-hidden flex-1 glass border transition-all duration-500 ${
      ...
      {isAudio ? (
        <div className={`absolute inset-0 flex flex-col items-center justify-center gap-6 ${
          isYou ? 'bg-gradient-to-br from-[#0a1a1a] to-[#0d1f2d]' : 'bg-gradient-to-br from-[#0f0a1e] to-[#150d2a]'
        }`}>
          <div className={`absolute inset-0 opacity-20 ${
            isYou
              ? 'bg-[radial-gradient(ellipse_at_center,_#14b8a6_0%,_transparent_70%)]'
          ...
          {isSearching && !isYou ? (
              ...
              <div className="relative flex items-center justify-center">
                {[1, 2, 3].map((i) => (
                  <div key={i} className={`absolute rounded-full border ${
                    ...
                    animation: `ripple 2s ease-out infinite`, animationDelay: `${i * 0.4}s`,
                  }} />
                ))}
                <div className={`w-20 h-20 rounded-full flex items-center justify-center border-2 ${
                  isYou ? 'bg-teal-500/10 border-teal-400/50' : 'bg-violet-500/10 border-violet-400/50'
                }`}>
                  <Radio size={28} className={isYou ? 'text-teal-400' : 'text-violet-400'} />
                </div>
              </div>
              <div className="text-center">
                ...
                <div className="flex justify-center gap-1 mt-1.5">
                  {['dot-1','dot-2','dot-3'].map(cls => (
                    <span key={cls} className={`${cls} w-1.5 h-1.5 bg-violet-400 rounded-full inline-block`} />
                  ))}
                </div>
              </div>
            </div>
            ...
            <div className="relative flex flex-col items-center gap-5 z-10">
              <div className="relative flex items-center justify-center">
                {[1, 2, 3].map((i) => (
                  <div key={i} className={`absolute rounded-full ${
                    ...
                    width: `${72 + i * 36}px`, height: `${72 + i * 36}px`,
                    animation: `pulse-ring ${1.2 + i * 0.3}s ease-in-out infinite`,
                    animationDelay: `${i * 0.2}s`,
                  }} />
                ))}
              ...
              <div className="flex items-end gap-1 h-8">
                {[3,6,9,12,9,6,3,6,9,6,3].map((h, i) => (
                  <div key={i} className={`w-1 rounded-full ${isYou ? 'bg-teal-400' : 'bg-violet-400'}`} style={{
                    height: `${h}px`, animation: `wave-bar 1s ease-in-out infinite`,
                    animationDelay: `${i * 0.08}s`, opacity: isYou ? 0.7 : 0.5,
                  }} />
                ))}
              </div>
              <p className={`text-xs font-medium ${isYou ? 'text-teal-300' : 'text-violet-300'}`}>
          ...
          <div className={`absolute top-3 left-3 flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-semibold glass border ${
            ...
            {isMuted ? <MicOff size={10} className="text-red-400" /> : <Mic size={10} />}
            {label} · Audio
          </div>
          ...
          <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-xs text-gray-400">
                {isYou ? (isMuted ? 'Muted' : 'Mic Live') : isSearching ? 'Searching...' : 'Connected'}
              </span>
            </div>
            {isYou && (
              <div className="p-1 glass rounded-lg border border-white/10">
        ...
        <div className="absolute inset-0 bg-gradient-to-br from-slate-900 to-slate-950 flex items-center justify-center">
          ...
          {stream && !isCamOff && (
            <video
              ...
              autoPlay
              playsInline
              muted={isYou}   // always mute your own preview to avoid echo
              className="absolute inset-0 w-full h-full object-cover"
            />
          )}

          {/* Overlay when no stream or cam off */}
          {(!stream || isCamOff) && (
            isYou ? (
              <div className="flex flex-col items-center gap-3 z-10">
                <div className="w-20 h-20 rounded-full bg-gradient-to-br from-teal-500/30 to-teal-600/20 border border-teal-500/40 flex items-center justify-center">
                  {isCamOff ? <CameraOff size={32} className="text-red-400" /> : <Camera size={32} className="text-teal-400" />}
                </div>
                <p className="text-gray-400 text-sm">{isCamOff ? 'Camera off' : 'Camera preview'}</p>
              </div>
            ) : isSearching ? (
              <div className="flex flex-col items-center gap-4 z-10">
                <div className="relative">
                  ...
                  <div className="absolute inset-0 rounded-full border border-violet-400/20 animate-spin" style={{ animationDuration: '3s' }}>
                    <div className="absolute -top-1 left-1/2 w-2 h-2 bg-violet-400 rounded-full -translate-x-1/2" />
                  </div>
                </div>
                <div className="text-center">
          ...
          <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between z-20">
            <div className="flex items-center gap-1.5">
              <div className={`w-2 h-2 rounded-full ${isYou ? 'bg-teal-400' : 'bg-violet-400'} animate-pulse`} />
              <span className="text-xs text-gray-400">
                {isYou ? (isCamOff ? 'Cam off' : 'Live') : isSearching ? 'Searching...' : 'Offline'}
              </span>
            </div>
            {isYou && (
              <div className="flex items-center gap-1.5">
                ...
                <div className="p-1 glass rounded-lg border border-white/10">
                  {isCamOff ? <CameraOff size={12} className="text-red-400" /> : <Camera size={12} className="text-teal-400" />}
                </div>
              </div>
            )}
          </div>
